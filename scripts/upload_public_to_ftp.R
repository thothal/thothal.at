...
.cli_quiet <- FALSE
cli_info <- function(...) {
  if (!.cli_quiet) cli_alert_info(...)
  invisible(NULL)
}

cli_success <- function(...) {
  if (!.cli_quiet) cli_alert_success(...)
  invisible(NULL)
}

cli_warning2 <- function(...) {
  if (!.cli_quiet) cli_alert_warning(...)
  invisible(NULL)
}

cli_danger2 <- function(...) {
  if (!.cli_quiet) cli_alert_danger(...)
  invisible(NULL)
}
...
upload_webpage <- function(service = .service_name, root = .public, username = NULL, password = NULL) {
  cli_h2_("Upload Website")
  files <- get_all_files(root)
  n_files <- length(files)
  cli_info("Found {n_files} file{?s}")
  ...
  for (file in files) {
    tryCatch(
      ...
      error = function(err) {
        ...
        cli_danger2("Error uploading file {.file {file}}")
      }
    )
    progress_update(pb)
  }
  ...
  pb$done()
  cli_success("{n_files - length(err_files)} file{?s} uploaded in [{timestamp}]")
  if (length(err_files)) {
    cli_warning2(
...
build_website <- function(build_rmd = TRUE, quiet = FALSE) {
  ...
  cli_success("Website built in [{timestamp}]")
}
